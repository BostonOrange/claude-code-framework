---
name: improve
description: Self-improvement — analyze the project and update AGENTS.md, rules, settings, and agents to improve AI effectiveness
---
...
If `--scope agents-md` or full pass: apply changes. Otherwise skip.

### Phase 3: Rules Improvement

For each rule file in `.codex/rules/`:
...
Read `.codex/settings.local.json` and check:
...
For each agent in `.codex/agents/`:
...
### Next Steps
1. Review changes in AGENTS.md and fill remaining placeholders
...
| Scenario | Behavior |
|----------|----------|
| AGENTS.md doesn't exist | Create from template, fill what's discoverable |
...
| Monorepo with multiple apps | Detect workspace structure, note in AGENTS.md |
