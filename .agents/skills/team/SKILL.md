...
/team full            — All 10 review/implementation agents (excludes meta-agents)
...
| `framework-improver` | Self-improvement of .codex/ config (invoked by `/improve`, not `/team`) | opus | Read/Write |
...
| `full` | all 10 review/implementation agents (excludes framework-improver) |
...
- Set `subagent_type` to the agent name from `.codex/agents/`
...
- `/improve` — Uses framework-improver agent to update .codex/ configuration
