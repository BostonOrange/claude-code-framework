...
3. Both files reference `docs/applier-pattern.md` for the canonical gate template, manifest format, recovery bash, and smoke-check pattern. Don't restate them.
...
If a new pattern would enforce a different structural property (e.g., "the agent can't see secrets" — would require sandboxed bash), it's worth adding. Otherwise, fit the work into one of the three.
