...
### Worked example: the multi-layer failure

Consider a polling dispatcher that drains a DB-backed outbox table and POSTs each pending row to a downstream integration that ultimately reaches an external billing/invoicing system. Two app servers concurrently `SELECT` the same `pending` row and both POST it, producing a duplicate downstream charge. Trace why nothing caught it: the producer's query had no `FOR UPDATE` (no atomic claim); the message send set no stable `MessageId` (no broker dedup key); the broker queue had `requiresDuplicateDetection: false` (broker dedup disabled); the consumer had no unique index on the idempotency key (no receiver enforcement). *Every* layer of defense was absent — any single one would have caught the duplicate. This is why the pattern mandates **both** producer claim and receiver idempotency: defense in depth, because in real systems layers get misconfigured one at a time.
