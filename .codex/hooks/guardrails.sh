...
echo "$COMMAND" | grep -qE "sf org delete" && block "CAUTION: Salesforce org deletion detected."

# Vercel production deploy
echo "$COMMAND" | grep -qE "vercel (deploy|promote).*--prod" && block "CAUTION: Production deployment detected."

# Kubernetes apply/delete
echo "$COMMAND" | grep -qE "kubectl (apply|delete|scale)" && block "CAUTION: Kubernetes mutation detected."

# Terraform apply/destroy
echo "$COMMAND" | grep -qE "terraform (apply|destroy)" && block "CAUTION: Terraform state mutation detected."
...
echo "$COMMAND" | grep -qE "git reset --hard" && \
    block "CAUTION: Hard reset detected. This will discard uncommitted changes."

echo "$COMMAND" | grep -qE "git clean -[a-z]*f" && \
    block "CAUTION: Git clean with force flag detected. This will delete untracked files."
...
echo "$COMMAND_NORM" | grep -qE "rm[[:space:]]+-[a-zA-Z]*[rRfF][a-zA-Z]*[[:space:]]+\*([[:space:]]|$)" && \
    block "CAUTION: rm -rf * against current directory — confirm you're not in a parent dir." 1
...
echo "$COMMAND" | grep -qE "chmod[[:space:]]+-R[[:space:]]+[0-7]{3,4}[[:space:]]+(/|~|\\\$HOME)([[:space:]/]|$)" && \
